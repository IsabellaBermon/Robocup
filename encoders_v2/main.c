...
int16_t angleSubtraction(int16_t angle)
{
...
void distanceRobotForward0()
{
  ...
  printf("Angulo0 %d\n", angle);
  if (angleSubtraction(angle) >= 350 && banTurns0)
  {
    turns0++;
    banTurns0 = false;
  }
  else if (angleSubtraction(angle) <= 20)
  {
    banTurns0 = true;
  }
  turnsToDistance0();
  //printf("Vueltas: %d, Angulo: %d\n", turns0, angleSubtraction(angle));
}
...
int main()
{
  ...
  i2c_init(AS560_i2c, 400000);           // Velocidad de Hz
  ...
  while (1)
  {
    ...
    //   pwm_set_chan_level(slice_num_5, PWM_CHAN_A, 750);
    //   pwm_set_chan_level(slice_num_5, PWM_CHAN_B, 750);
    //   angleOffset = angleSubtraction(getAngle0());
    //   turns0 = 0;
    //   distance0 = 0;
    //   turns1 = 0;
    //   distance1 = 0;
